...
if __name__ == '__main__':
    ...
    # Load generated data
    results_fn_list = glob(os.path.join(args.sample_path, '*result_*.pt')) # FIXME: was *result_*.pt 

    if args.eval_num_examples is not None:
        results_fn_list = results_fn_list[:args.eval_num_examples]
    new_results_fn_list = []
    idx_fn = set(list(range(args.start_data_id,args.end_data_id)))
    for j in results_fn_list:
        if int(j[-5:-3].replace('_', '')) in idx_fn:
            print(int(j[-5:-3].replace('_', '')))
            new_results_fn_list.append(j)

    logger.info(f'evaluating file list {new_results_fn_list}')
    num_examples = len(new_results_fn_list)
    ...
    for example_idx, r_name in enumerate(tqdm(new_results_fn_list, desc='Eval')):
        if debug:
            print(r_name)
        ...
        all_pred_ligand_v = []
        for item in r:
            all_pred_ligand_pos.append(item['pred_pos_traj'])
            all_pred_ligand_v.append(item['pred_v_traj'])
        ...
        for sample_idx, (pred_pos, pred_v) in enumerate(zip(all_pred_ligand_pos, all_pred_ligand_v)):
            ...
            try:
                ...
                if debug:
                    print(mol)
                    w = Chem.SDWriter('ill_mol_in_decomp.sdf')
                    # mol.AddConformer(conformer_i)
                    w.write(mol)
                    w.close()
                    mol = Chem.SDMolSupplier('ill_mol_in_decomp.sdf')[0]
            ...
            try:
                ...
                if args.docking_mode == 'qvina':
                    vina_task = QVinaDockingTask.from_generated_mol(
                        mol, r[0]['ligand_filename'] , protein_root=args.protein_root)
                    vina_results = vina_task.run_sync()
                elif args.docking_mode in ['vina_score', 'vina_dock']:
                    if debug:
                        print("entering vina cycle!")
                        print("show ligand filename",r[0]['ligand_filename'], args.protein_root)
                    vina_task = VinaDockingTask.from_generated_mol(
                        mol, r[0]['ligand_filename'] , protein_root=args.protein_root)
                    if debug:
                        print("successfully init the vina dock",args.exhaustiveness)
                    ...
                    if debug:
                        print("got the vina results",vina_results)
            ...
            results.append({
                'mol': mol,
                'smiles': smiles,
                'ligand_filename': r[0]['ligand_filename'],
                'pred_pos': pred_pos,
                'pred_v': pred_v,
    ...
    if args.docking_mode == 'qvina':
        ...
        if debug:
            print("was to print out vina results")
