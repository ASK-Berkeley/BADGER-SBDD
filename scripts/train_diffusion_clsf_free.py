...
from models.molopt_score_model_clsf_free_guide import ScorePosNet3D 
...
if __name__ == '__main__':
    ...
    if args.wandb:
        wandb.init(
            ...
            project="visualizing training process for main diffusion model",
            name=f"group info drop out rate {config.train.p_uncond}", 
            # track hyperparameters and run metadata
            config={
                "batch_size": config.train.batch_size,
                "dropout_rate": config.train.p_uncond
            }
        )
    ...
    print("show device",args.device)
    model = ScorePosNet3D(
        config.model,
        protein_atom_feature_dim=protein_featurizer.feature_dim,
        ligand_atom_feature_dim=ligand_featurizer.feature_dim,
        p_dropout=config.train.p_uncond,
        null_indicator=config.train.null_indicator
    ).to(args.device)
    # print("diffusion",model.refine_net)
    ...
    # hyperparameter
    p_uncond = config.train.p_uncond
    ...
    def train(it,args):
        ...
        for _ in range(config.train.n_acc_batch):
            ...
            batch_binding_aff = batch.y.reshape(-1,1)
            ...
            results = model.get_diffusion_loss(
                ...
                bind_aff = batch_binding_aff, # here, feeding in the class label
            ...
            if args.wandb:
                wandb.log({"total loss [train]": loss,
                        "loss_pos [train]":loss_pos,
                        "loss_v [train]":loss_v,
                        "lr":optimizer.param_groups[0]['lr'],
                        #    "Grad Norm":orig_grad_norm
                        })
    ...
    def validate(it,args):
            ...
            for batch in tqdm(val_loader, desc='Validate'):
                ...
                for t in np.linspace(0, model.num_timesteps - 1, 10).astype(int):
                    ...
                    results = model.get_diffusion_loss(
                        ...
                        p_uncond=p_uncond, # classifier free
                        train=True
                    )
                    loss, loss_pos, loss_v = results['loss'], results['loss_pos'], results['loss_v']
        ...
        if args.wandb:
            wandb.log({
                "avg_loss":avg_loss,
                "avg_loss_pos":avg_loss_pos,
                "avg_loss_v * 1000":avg_loss_v * 1000,
                "Avg atom auroc": atom_auroc
            })
