...
def sample_diffusion_ligand(model, data, num_samples, batch_size=16, device='cuda:0',
                            ...
                            sample_num_atoms='prior',context=None, s=0, s_v=0, classifier=None, enable_wandb=False,clip=1e6):
...
if __name__ == '__main__':
    ...
    for id in list(range(args.start_data_id,args.end_data_id)):
        ...
        result_path = args.result_path+"s="+str(int(config.sample.s))+"c="+str(int(config.sample.context))+"clip="+str(config.sample.clip)+"fixcontext_trained_x0"
