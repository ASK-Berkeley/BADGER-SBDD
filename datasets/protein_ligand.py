...
def get_ligand_atom_features(rdmol):
    ...
    feat_mat = np.array([atomic_number, aromatic, degree, num_hs, hybrid], dtype=np.int_).transpose()
    # FIXME: (Yue Jian) originally was np.long,but it will cause failure, so change to np.int_
...
def parse_sdf_file_mol(path, heavy_only=True, mol=None):
    # print("entering parse function")
    if mol is None:
        if path.endswith('.sdf'):
            # print("before mol from file")
            mol = Chem.MolFromMolFile(path, sanitize=False)
            # print("after mol from file")
    ...
    element = np.array(element, dtype=np.intc)
    # element = np.array(element, dtype=np.int)
    # FIXME:(YUE JIAN) same as np.long problem above
    pos = np.array(pos, dtype=np.float32)
    row, col, edge_type = [], [], []
    BOND_TYPES = {t: i for i, t in enumerate(BondType.names.values())}
    ...
    edge_index = np.array([row, col], dtype=np.intc)
    # dge_index = np.array([row, col], dtype=np.long)
    # FIXME: same(YJ)
    edge_type = np.array(edge_type, dtype=np.intc)
    # edge_type = np.array(edge_type, dtype=np.long)
    # FIXME: same(YJ)
    perm = (edge_index[0] * num_atoms + edge_index[1]).argsort()
    edge_index = edge_index[:, perm]
