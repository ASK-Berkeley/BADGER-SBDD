...
class ScorePosNet3D(nn.Module):
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         ...
                         num_steps=None, center_pos_mode=None, pos_only=False, context=None, s=0, s_v=0, classifier=None,clip=1e6 ,enable_wandb=False):
        """
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            with torch.enable_grad():
                with RequiresGradContext( ligand_pos, requires_grad=True):
                    ...
                    grad_pos = autograd.grad(energy.sum(), ligand_pos, retain_graph=True)[0]
