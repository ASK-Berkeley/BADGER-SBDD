...
from utils.sample import RequiresGradContext, l2_square, l1_loss

debug = False
debug_v = False
create_fix_seed = False
load_fix_seed = False
...
class ScorePosNet3D(nn.Module):
    ...
    # TODO: add this back
    # @torch.no_grad()
    def sample_diffusion_multi(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand,
                         num_steps=None, center_pos_mode=None, pos_only=False, 
                         dock_context=None, qed_context=None, sa_context=None, s=0, s_v=0, 
                         classifier=None, classifier_sa=None,
                         clip = 10.0, sample_loss_type = "L2", enable_wandb=False):
        """
        ...
        # the position of ligand and pocket have been recentered before the sampling start
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            with torch.enable_grad():
                with RequiresGradContext(ligand_pos, requires_grad=True):
                    ...
                    preds = self(
                        ...
                        init_ligand_pos=ligand_pos, # should feed this term into classifier
                        init_ligand_v=ligand_v,
                        batch_ligand=batch_ligand,
                        time_step=t
                    )
                    ...
                    # mu_t, guidance is added to this term
                    ...
                    if enable_wandb:
                        wandb.log({
                            ...
                            "sigma_t [mean]":torch.mean((0.5 * pos_log_variance).exp()),
                            "mu_t(x_t,x_0) [max]": torch.max(pos_model_mean),
                            "sigma_t [max]": torch.max((0.5 * pos_log_variance).exp()),
                            "mu_t(x_t,x_0) [min]": torch.min(pos_model_mean),
                            "sigma_t [min]": torch.min((0.5 * pos_log_variance).exp()),
                        })
                    # original
                    # prefactor = (self.betas[i]**1.5)/(torch.sqrt(1-self.alphas_cumprod[i])*torch.sqrt(1.-self.betas[i]))
                    # new
                    prefactor = (self.betas[i])/torch.sqrt(1.-self.betas[i])
                    if enable_wandb:
                        wandb.log({ "t":i,
                                    "beta_t":self.betas[i],
                    ...
                    prediction = self.classifier(
                        ...
                        ligand_v=ligand_v,
                        batch_ligand=batch_ligand,
                        time_step = t # good to go
                    )
                    predicted_affinities = prediction[:, 0:1]
                    predicted_qed = prediction[:, 1:2]
                    predicted_sa = prediction[:, 2:3]

                    prediction_for_log = torch.mean(prediction).item()

                    if enable_wandb:
                        wandb.log({"current binding affinity": torch.mean(predicted_affinities).item()})
                        wandb.log({"current qed": torch.mean(predicted_qed).item()})
                        wandb.log({"current sa": torch.mean(predicted_sa).item()})

                    dock_c = dock_context * torch.ones_like(predicted_affinities)
                    qed_c = qed_context * torch.ones_like(predicted_affinities)
                    sa_c = sa_context * torch.ones_like(predicted_affinities)

                    if sample_loss_type == "L2":
                        dock_loss = l2_square(predicted_affinities, dock_c)
                        qed_loss = l2_square(predicted_qed, qed_c)
                        sa_loss = l2_square(predicted_sa, sa_c)
                    elif sample_loss_type == "L1":
                        dock_loss = l1_loss(predicted_affinities, dock_c)
                        qed_loss = l1_loss(predicted_qed, qed_c)
                        sa_loss = l1_loss(predicted_sa, sa_c)
                    else:
                        raise ValueError("Incorrect sampling loss type.")

                    total_loss = dock_loss + qed_loss + sa_loss

                    grad_pos = autograd.grad(total_loss.sum(), ligand_pos, retain_graph=True)[0]

                if enable_wandb:
                    wandb.log({
                        "dock_loss": torch.mean(dock_loss),
                        "qed_loss": torch.mean(qed_loss),
                        "sa_loss": torch.mean(sa_loss),
            ...
            with torch.no_grad():
                clip_guidance = torch.clip(s * prefactor * grad_pos, min=-clip, max=clip)
                ligand_pos_next = pos_model_mean + nonzero_mask * (0.5 * pos_log_variance).exp() * torch.randn_like(
                    ligand_pos) - clip_guidance
                ...
                if enable_wandb:
                    wandb.log({
                        ...
                        "s*prefactor*grad_pos [mean]":torch.mean(s * prefactor * grad_pos),
                        "s*prefactor*grad_pos [max]": torch.max(s * prefactor * grad_pos),
                        "s*prefactor*grad_pos [min]": torch.min(s * prefactor * grad_pos),
                    })
