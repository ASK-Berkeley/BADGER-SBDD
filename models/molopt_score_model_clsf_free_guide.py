...
# Yue Jian: code template borrowed from DiT code base
class ContinuousLabelEmbedder(nn.Module):
    """
    Embeds continuous conditional labels into vector representations. Also handles label dropout for classifier-free guidance.
    """
    def __init__(self, feat_mix_dim, hidden_size, dropout_prob,null_indicator = None):
        super().__init__()
        self.dropout_prob = dropout_prob
        self.cond_emb = nn.Sequential(
                    nn.Linear(1, feat_mix_dim * 2),
                    nn.GELU(),
                    nn.Linear(feat_mix_dim * 2, hidden_size)
                )
    ...
    def token_drop(self, conditions, force_drop_ids=None):
        ...
        conditions = torch.where(drop_ids.reshape(-1,1), self.null_indicator, conditions) # set -20. on binding affinity if no conditioning
        return conditions

    def forward(self, conditions, train=False, force_drop_ids=None):
        use_dropout = self.dropout_prob > 0
        # embeddings = self.cond_emb(conditions)
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim, p_dropout=0.,null_indicator = None):
        super().__init__()
        self.config = config
        ...
        # time embedding & conditional embedding
        ...
        if self.time_emb_dim > 0:
            if self.time_emb_mode == 'simple':
                ...
                self.cond_emb = ContinuousLabelEmbedder(
                                                        feat_mix_dim=config.cond_emb_dim,
                                                        hidden_size=self.time_emb_dim,
    ...
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
                time_step=None, return_all=False, fix_x=False, condition = None, p_uncond = 0.,train=False):
        ...
        if not train:
            ...
            condition = torch.cat(batch_size*[condition],0) 
        ...
        if self.time_emb_dim > 0:
            if self.time_emb_mode == 'simple':
                cond_feat = self.cond_emb(condition,train=train)
                combine_feat = cond_feat # we now disable time feat
                input_ligand_feat = torch.cat([
                    init_ligand_v,
                    combine_feat[batch_ligand]
                ], -1)
            elif self.time_emb_mode == 'sin':
                cond_feat = self.cond_emb(condition,train=train)
                combine_feat = cond_feat[batch_ligand] # we now disable time feat
                input_ligand_feat = torch.cat([
                    init_ligand_v, 
                    combine_feat
                ], -1)
    ...
    def q_v_posterior(self, log_v0, log_vt, t, batch):
        ...
        unnormed_logprobs = log_qvt1_v0 + self.q_v_pred_one_timestep(log_vt, t, batch) # log q(vt | vt-1, x0) + log q(vt-1 | x0)
    ...
    def get_diffusion_loss(
            ...
            time_step=None, bind_aff=None, p_uncond=0., train=False
    ):
        ...
        preds = self(
            ...
            condition = bind_aff, # feed in binding affinity as conditioning
            p_uncond = p_uncond,
            train=train
        )
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                        ...
                        num_steps=None, center_pos_mode=None, pos_only=False, context=None, s=0, s_v=0, classifier=None,clip=1e6 ,enable_wandb=False, condition=0. ,w = 0.,norm_type="l2"):
        """
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            preds = self(
                ...
                batch_ligand=batch_ligand,
                time_step=t,
                condition=torch.tensor([-20.]).to("cuda")

            )

            # Yue Jian, classfier free guidance
            # condition x_0
            preds_c = self(
                protein_pos=protein_pos,
                protein_v=protein_v,
                ...
                batch_ligand=batch_ligand,
                time_step=t,
                condition=torch.tensor([condition]).to("cuda")
            )
            ...
            if self.model_mean_type == 'noise':
                ...
                pred_pos_noise_c = preds_c['pred_ligand_pos'] - ligand_pos

                eps_prime = (1 + w)* pred_pos_noise_c - w * pred_pos_noise
                # TODO: linearly combine the two noise score here to do classifier free guidance
                pos0_from_e = self._predict_x0_from_eps(xt=ligand_pos, eps=eps_prime, t=t, batch=batch_ligand)
                v0_from_e = preds['pred_ligand_v']
            elif self.model_mean_type == 'C0':

                # Yue Jian: bug fixed, here is the actual mode of model
                pos0_from_e = preds['pred_ligand_pos']
                pos0_from_e_c = preds_c['pred_ligand_pos']
                # be cautious with this equivalent linear combination of two score model predictions
                pos0_from_e_prime = (1 + w)* pos0_from_e_c - w * pos0_from_e

                v0_from_e = preds['pred_ligand_v']
            else:
                raise ValueError

            pos_model_mean = self.q_pos_posterior(x0=pos0_from_e_prime, xt=ligand_pos, t=t, batch=batch_ligand)
            ...
            if enable_wandb:
                wandb.log({
                    "change in ligand pos x_t - x_{t-1} [mean]":torch.mean(delta_pos),
                    "change in ligand pos x_t - x_{t-1} [max]": torch.max(delta_pos),
                    "change in ligand pos x_t - x_{t-1} [min]": torch.min(delta_pos),
            ...
            if not pos_only:
                ...
                assert ligand_v.max().item() < self.num_classes, f'Error: {ligand_v.max().item()} >= {self.num_classes}'
                ...
                if enable_wandb:
                    wandb.log({
                        ...
                        # "grad_v [mean]":torch.mean(grad_v.float()),
                        "delta_one_hot_ligand_v [max]": torch.max(ligand_v_onehot - F.one_hot(ligand_v, self.num_classes).float())# should be preserve since there is argmax every step
                    })
