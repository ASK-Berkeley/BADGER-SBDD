...
class EnBaseLayer(nn.Module):
    ...
    def forward(self, h, x, edge_index, mask_ligand, edge_attr=None):
        ...
        rel_x = x[dst] - x[src]
        # if debug:
        #     print("rel_x",rel_x.shape,x.shape,x[dst].shape,x[src].shape)
        d_sq = torch.sum(rel_x ** 2, -1, keepdim=True)
        if debug:
            print("rel_x",rel_x.shape,x.shape,x[dst].shape,x[src].shape)
            a = torch.sqrt(d_sq + 1e-8)
            print("d_sq",d_sq.shape,a.size())
        ...
        if self.update_x:
            ...
            if debug:
                print("(xi - xj)",(xi - xj).shape)
                print("(torch.sqrt(d_sq + 1e-8) + 1)",(torch.sqrt(d_sq + 1e-8) + 1).shape)
                print("mij",mij.shape)
                print("self.x_mlp(mij)",self.x_mlp(mij).shape)
                print("dst",dst.shape)
            delta_x = scatter_sum((xi - xj) / (torch.sqrt(d_sq + 1e-8) + 1) * self.x_mlp(mij), dst, dim=0)
            if debug:
                print("x",x.shape, "delta_x",delta_x.shape, 'mask_ligand', mask_ligand.shape)
...
class EGNN(nn.Module):
    def __init__(self, num_layers, hidden_dim, edge_feat_dim, num_r_gaussian, k=32, cutoff=10.0, cutoff_mode='knn',
                 update_x=True, act_fn='silu', norm=False, device = 'cpu'):
        super().__init__()
        # Build the network
    ...
    def _connect_edge(self, x, mask_ligand, batch):
        ...
        if self.cutoff_mode == 'knn':
            if debug:
                ...
                # print("show ei",edge_index.shape)
                # edge_index = torch.ones((2,1928))
                # edge_index = edge_index.to(self.device)
                # x = x.to(self.device)
                # print("pass this part",edge_index.device)
            else:
                # temp_x = x.to('cuda:0')
                # temp_batch = batch.to("cuda:0")
                # edge_index = knn_graph(temp_x, k=self.k, batch=temp_batch, flow='source_to_target')
                edge_index = knn_graph(x, k=self.k, batch=batch, flow='source_to_target')
                # edge_index = edge_index.to(self.device)
                # x = x.to(self.device)
                # del temp_x, temp_batch
