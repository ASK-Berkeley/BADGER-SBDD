...
def create_index_for_sampling(path, save_dir):
    """
    creating index.pkl file for prepare ligand and protein pair data
    Args:
        path: root path for sample ligand
        save_dir: the file that contain both .pdb and .sdf

    Returns: None

    """
    if not os.path.isfile(path+"/index.pkl"):
        fn_list = os.listdir(os.path.join(path, save_dir))
        pocket_n = None
        for fn in fn_list:
